...
- 历史输入浮层:输入框上方「历史输入」入口打开浮层,浏览并回填同一 dsh 工作区(cwd)下所有会话的用户输入。数据权威来源为会话产物:host 半区只读路由 `GET /api/session-manager/inputs?sessionId=` 以当前会话反查 cwd,经官方 `sessionQuery.readSession` 并行聚合同工作区最近 20 个会话的 `user/message` 事件(仅 `source.kind === 'user'` 的人类输入,工具结果回填与插件注入排除),文本精确去重保留最新时间、按时间倒序、上限 200 条、单条截 20000 字符;host 侧 cwd 级缓存 TTL 5 分钟 + 按 cwd 键控的 in-flight 去重,单会话产物读取失败仅告警跳过。client 半区挂官方 `conversation.input.dock` 插槽(dsh-client-ui-goal 同构先例),浮层每次打开即强刷;条目点选回填,浮层内 `↑/↓` 选择、`Enter` 填入、`Esc` 关闭(捕获阶段拦截,先于 Lexical 光标移动;IME 组合中跳过);回填一律走宿主公共契约 `inputActions.setDraft`(不直改编辑器 DOM)。宿主缺该插槽或 `inputActions` 缺失时本功能整体不注册不渲染,不影响其余能力。
...
- 历史输入的已知取舍:扫描范围限同 cwd 最近 20 个会话(全量解压在大会话量下不可接受,近期会话覆盖回填真实需求),更早会话的输入不入浮层;浮层每次打开即强刷,刚发送的输入重开浮层立即可见;单条超长文本(大段粘贴)在 host 侧截断,回填与展示均用截断后文本;交互收敛为浮层浏览点选(不做全局快捷键循环回溯:web 环境可发现性差、与编辑器光标移动和系统键冲突面大、readline 状态机复杂度高)。
