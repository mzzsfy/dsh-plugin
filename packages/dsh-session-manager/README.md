...
- 历史输入的已知取舍:对齐扫描各工作区最近 `HISTORY_SESSION_SCAN_LIMIT` 个会话(更早会话的输入不入缓存),解压发生在后台、永不阻塞浮层响应,代价是首次对齐期间列表为空、需等静默重拉;**解压不重复执行**:每会话提取结果持久化在工作区缓存 `extracts` 段并附产物 stat 指纹(mtimeMs+size),对齐时先 stat 比对、产物未变零解压,判断完全基于磁盘、跨重启生效;运行中会话(当前会话)提取结果只保留内存不落盘——产物持续变化,落盘指纹立即失效;启动分两拍:**当前会话历史立即就绪**(运行中会话即时对齐),全量启动对齐延迟 `HISTORY_STARTUP_DELAY_MS` 再跑且只回溯最近 `HISTORY_STARTUP_SCAN_LIMIT` 个会话覆盖的工作区;对齐中连续解压占用超 `HISTORY_ALIGN_SLICE_MS` 即让出 `HISTORY_ALIGN_YIELD_MS`(zstd 解压为同步 CPU 操作,不设让出会让宿主启动阻塞数十秒);不解压搬进 worker:产物为多帧 zstd + 帧扫描 + 记录解码 + 版本协商,`readSession` 深度耦合宿主持久层,worker 内复刻等于维护第二份产物解析实现,宿主升级即漂移;缓存上限 `HISTORY_INPUT_LIMIT` 条,超出裁掉最旧(超长会话史会丢早期输入);运行中会话输入经对齐进入缓存有节流延迟(`HISTORY_ALIGN_THROTTLE_MS`),非严格实时;缓存文件为明文 JSON(输入内容落盘 `~/.dsh/historyPrompt`,与产物本身同级敏感);单条超长文本(大段粘贴)在 host 侧截断,回填与展示均用截断后文本;交互收敛为快捷键唤起浮层(`Alt+↑`,无 mac 系统占用,与编辑器光标移动无冲突),无常驻界面元素。
