...
- 历史输入的已知取舍:对齐扫描各工作区最近 `HISTORY_SESSION_SCAN_LIMIT` 个会话(更早会话的输入不入缓存),解压发生在后台、永不阻塞浮层响应,代价是首次对齐期间列表为空、需等静默重拉;**启动对齐只回溯最近 `HISTORY_STARTUP_SCAN_LIMIT` 个会话覆盖的工作区**,多工作区串行排队、每批 `HISTORY_ALIGN_BATCH_SIZE` 个会话解压后停 `HISTORY_ALIGN_BATCH_PAUSE_MS` 让出主循环(zstd 解压为同步 CPU 操作,不设批会让宿主启动阻塞数十秒);缓存上限 `HISTORY_INPUT_LIMIT` 条,超出裁掉最旧(超长会话史会丢早期输入);运行中会话输入经对齐进入缓存有节流延迟(`HISTORY_ALIGN_THROTTLE_MS`),非严格实时;缓存文件为明文 JSON(输入内容落盘 `~/.dsh/historyPrompt`,与产物本身同级敏感);单条超长文本(大段粘贴)在 host 侧截断,回填与展示均用截断后文本;交互收敛为快捷键唤起浮层(`Alt+↑`,无 mac 系统占用,与编辑器光标移动无冲突),无常驻界面元素。
