...
const SETTINGS_SCHEMA = z.object({
  webhookUrl: z.string().default('').description('webhook 目标 URL(Slack-compatible {text}),留空禁用'),
  minTurnDurationMs: z.number().default(5 * 1000).description('回合最短时长过滤,毫秒,仅作用于 turn/end 类'),
...
const readSettings = (ctx) => {
  ...
  const fallback = { webhookUrl: '', minTurnDurationMs: 5 * 1000, rootsOnly: true, enabled: {}, soundMapping: {} }
  ...
  ctx.effect(() =>
    ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          const body = JSON.parse((await readRawBody(req, 64 * 1024)).toString('utf8'))
