...
开启面板"通知"页的宿主通知开关后,通知触发时由宿主进程直接弹操作系统级桌面通知,零依赖:macOS 走 osascript,Linux 走 notify-send,Windows 走 PowerShell 调 WinRT toast;超时 10 秒,失败即弃不重试,不影响其余通道。两个开关:

- **宿主机弹桌面通知**:通知触发即弹。与本机浏览器**去重**:以回环地址(127.0.0.1/::1)访问宿主的浏览器窗口视为本机浏览器,其在线时宿主让位,由浏览器负责本机呈现(点击直达、会话高亮等浏览器通知体验更完整);本机浏览器全关、或只有远程设备浏览器在线时,宿主才弹——远程浏览器的通知弹在远程设备,不与本机重复。
- **仅当本机无浏览器接收时补位**(回退):同样按本机浏览器在线与否判定,在线不弹、离线补弹;总开关开启时本开关冗余。

"本机浏览器在线"按投影长轮询判定:窗口在服务端挂起上限加余量(约 30 秒)内有轮询即算在线;浏览器刚关闭的这段窗口期内事件不弹宿主,属启发式留白。经本机反向代理转发访问时所有客户端的来源恒为回环,一律判"本机在场",宿主通知恒让位(失败方向安全:宁可不弹不误弹)。宿主在无桌面的服务器上时 spawn 会静默失败,无害;测试页"测试宿主通知"返回真实结果,可据此验证宿主机桌面环境是否可用。
...
turn-notify:
  ...
  hostNotify: false               # 宿主机桌面通知总开关;本机浏览器在线时让位(同机去重)
  hostNotifyFallback: false       # 仅当本机无浏览器窗口在线接收时才弹宿主桌面通知
  enabled:                        # 六类事件独立开关
    completed: true
    error: true
    interrupted: true
...
3. **host 直发通道**:webhook、IM 与宿主桌面通知由 host 进程直接投递(经路由名单过滤),不依赖浏览器;宿主通知按本机(回环来源)浏览器在线与否去重让位或回退补位;同时通知写入内存队列(环形 20 条,60 秒过期,不落盘),路由名单随单元下发。
...
- 宿主桌面通知的"本机浏览器在线"按投影轮询近似(挂起上限加余量约 30 秒),浏览器刚关闭的窗口期内事件不弹宿主;本机窗口在线但呈现偏好全关(声音/弹窗/页内全关或系统通知未授权)时宿主同样让位,该事件本机无 OS 级通知(浏览器侧仅剩会话行高亮);无桌面的宿主上 spawn 静默失败,无害。在场记账的形态校验(回环 + 续传 cursor + 同源)为启发式,防无意探活误压与网页伪造在场,不构成对抗本机进程的安全边界。
