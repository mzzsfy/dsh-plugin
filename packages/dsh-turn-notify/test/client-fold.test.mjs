...
const PURE_END = CLIENT_SRC.indexOf('// ---- 点击直达会话')
...
// 通知长轮询经挂起 fetch 静默(测试只驱动 fold effect,不消费投影)
function loadClient() {
  const modules = []
  const effects = []
  const windowStub = {
  ...
  // 不传 fetch 工厂参数:源码内裸 fetch 解析到 globalThis.fetch,由 withFetch 显式 mock;
  // 传参数会把 fetch 绑定进沙箱,mock 永远打不到
  ...
  mod.apply(ctx)
  const entry = effects.find((item) => item.tag === 'turn-notify folder running mark')
...
async function withFetch(implOrPayload, run) {
  ...
  // 成功响应 {folderRunningEnabled:true}——默认路径显式 mock,不依赖运行环境对相对
  // URL 的 fetch 行为(隐式环境耦合会随 Node 升级漂移);
  // 通知长轮询路径恒返回挂起 Promise,测试期间不产生投影循环
  const impl = typeof implOrPayload === 'function'
    ? async (path) => (String(path).indexOf('/api/turn-notify/projection') === 0 ? new Promise(() => {}) : implOrPayload(path))
    : async (path) => {
      if (String(path).indexOf('/api/turn-notify/projection') === 0) return new Promise(() => {})
      return { ok: true, json: async () => (implOrPayload === undefined ? { folderRunningEnabled: true } : implOrPayload) }
    }
...
test('Given 开关关闭(拉取返回 folderRunningEnabled:false), When 扫描, Then 不标记且既有标记清除', async () => {
  await withFetch({ folderRunningEnabled: false }, async () => {
    const client = loadClient()
    baseSnapshots(client)
...
test('Given 拉取未完成即卸载, When 后续拉取完成并触发调度, Then 标记不复活', async () => {
  ...
  await withFetch((path) => {
    ...
    cleanup()
    release({ ok: true, json: async () => ({ folderRunningEnabled: true }) })
    await sleep(SCAN_SETTLE_MS)
...
test('Given 开关停用, When 拉取完成前的首次扫描, Then 不产生标记(未决期仅清除)', async () => {
  ...
  await withFetch((path) => new Promise((resolve) => {
    if (String(path).indexOf('/api/turn-notify/projection') === 0) return
    setTimeout(() => resolve({ ok: true, json: async () => ({ folderRunningEnabled: false }) }), pure.scanDelayMs * 3)
  }), async () => {
    const client = loadClient()
    baseSnapshots(client)
    const s1 = sectionStub()
...
// ── 源码契约:CSS 规则、面板开关挂载、选择器与标记属性 ──

test('源码契约:运行点与图标着色规则、官方组容器锚点、面板开关挂载齐备', () => {
  assert.ok(CLIENT_SRC.includes('div[class$="_groupSection"][data-tn-folder-running]'), '缺组容器标记 CSS 锚点')
  assert.ok(CLIENT_SRC.includes('var(--dsw-static-deepseek-450)'), '运行点色源应为官方 ongoing 实际色值(--dsh-state-ongoing 仅存在于官方状态点组件局部,组容器读不到)')
  assert.ok(!CLIENT_SRC.includes('--tn-fold-running-color: var(--dsh-state-ongoing'), '死变量引用:组容器上下文读不到 --dsh-state-ongoing,恒走回退源,不得引入永不生效的引用')
  assert.ok(CLIENT_SRC.includes('[class$="_folderActive"]'), '图标着色应让位官方 folderActive 态')
  assert.ok(CLIENT_SRC.includes('tn-fold-running-pulse'), '缺运行点脉冲动画')
  assert.ok(CLIENT_SRC.includes('prefers-reduced-motion'), '动画应尊重减少动态偏好')
  assert.ok(CLIENT_SRC.includes('toggleFolderRunning'), '面板缺运行标记开关处理函数')
  assert.ok(CLIENT_SRC.includes('title: FOLD_SWITCH_TITLE'), '开关行缺悬停说明挂载')
  assert.ok(CLIENT_SRC.includes('const FOLD_SWITCH_TITLE'), '缺悬停说明常量')
  // 开关经既有 config 路由读写:不新增路由,host 与 client 字面量一致
  assert.ok(!CLIENT_SRC.includes("'/api/turn-notify/folder-running"), '运行标记开关不得另开路由(经 config 路由读写)')
})
