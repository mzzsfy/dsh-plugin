...
对话增强插件:历史输入回溯、插话撤回、对话分叉(家族版本切换/进行中轮分叉/自动重发)。dsh-session-manager 历史输入功能的独立延续包(原包已移除该功能)。
...
设置页「插件」→「对话增强」卡片四开关(历史输入/插话撤回/对话分叉/分叉后自动重发),经宿主 settings 持久,变更刷新页面生效。浮层内 `S` 键或开关行亦可切换。
