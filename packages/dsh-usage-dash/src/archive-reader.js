...
// 现役宿主:list 返回 snapshot(id 在 .header),整读走 read 句柄。
// 信号包裹为 options 对象;read 失败仍保证 close,对齐官方 readColdSessionLog。
// 有意不附加 interruptedTurnClosers:crash 中断的 turn 不计 turn/end,
// 与两代既有统计口径一致,不为统计引入宿主模块依赖。
class HandleArchiveReader {
  ...
  async list(signal) {
    ...
    return snapshots.map((snapshot) => ({ id: snapshot.header.id }))
  }
...
class InspectArchiveReader {
  ...
  async list(signal) {
    ...
    return headers.map((header) => ({ id: header.id }))
  }

  async readLog(id, signal) {
    const inspection = await this.persistence.inspect(id, signal)
    return { inheritedEventCount: inspection.inheritedEventCount, events: inspection.events }
  }
}
