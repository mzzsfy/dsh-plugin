...
- 条件类型(数组内 AND,空数组恒生效),**所有范围条件统一左闭右开**(含起始不含结束,相邻区间无缝衔接,如 1~15 与 15~31 的 15 号归后者),编辑器支持添加/编辑/删除:每规则卡下方「+时段/+星期/+号段/+日期段」按钮即添加对应类型默认条件(时段默认全天 00:00~24:00、周几默认空、号段默认全月 1~32、日期段默认当天~次日),类型下拉切换即重置为该类型默认值;编辑器端校验拒绝非法时刻(需 HH:MM)、周几(需 0-6)、月号(需 1-31,结束号允许 32 表达"到月末")、非规范日期与空区间(from===to 恒不生效,不可保存)
