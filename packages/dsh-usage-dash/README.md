...
- 回扫状态行默认隐藏:首次启用自动回扫历史会话,运行中显示进度(常显);跳过会话与写入失败默认折叠为刷新左侧箭头标识,点击在其下方展开扫描异常日志块(汇总 + 时间/类型/明细逐条,上限 200 条,重建时清空),采集错误常显;工具栏分主区与右侧操作区,「刷新/重建」恒驻右上角,窄宽度时挡位组在主区内换行;「重建」二次确认(3 秒)后清库重扫
