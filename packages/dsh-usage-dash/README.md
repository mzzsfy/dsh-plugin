...
回扫经内部存档读取适配层(`src/archive-reader.js`)访问宿主 `sessionPersistence`:按能力检测在宿主 API 代际间分派(现役 `open('read')` 句柄式 / 旧代 `inspect` 一次整读),宿主再变只增适配器,回扫主体不动。旧格式会话存档文件(v0 起)由宿主迁移链在读路径统一转换为当前事件词汇,新旧多版本存档均可解析;个别无法读取的会话(未来格式、真损坏)跳过并记入扫描异常日志,不阻断整轮回扫。
