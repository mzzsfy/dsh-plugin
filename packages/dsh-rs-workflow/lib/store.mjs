// run-store:每 run 全量 JSON + index.json 索引;LRU 容量收敛;零截断
import { existsSync, mkdirSync, readFileSync, readdirSync, renameSync, rmSync, writeFileSync } from 'node:fs'
import { homedir, tmpdir } from 'node:os'
import { join } from 'node:path'

export const KEEP_RUNS = 200
export const ACTIVE_STATES = new Set(['running', 'paused'])
...
const emptyState = () => ({ status: 'running', steps: {}, approvals: {}, escalations: 0, queued: [], batchSeq: 0 })
...
export function createStore({ dir = defaultDataDir(), logger = console, keepRuns = KEEP_RUNS } = {}) {
  ...
  const store = {
    start({ runId, sessionId, workspace, request, templateId, inputs, state }) {
      ensureLoaded()
      ...
      const record = {
        ...
        summary: undefined, state: state ?? emptyState(), controls: [], steps: {}, queued: [],
      }
    ...
    step({ runId, stepId, instance, event, body }) {
      ...
      if (CONTROL_EVENTS.has(event)) {
        record.controls.push({ at, kind: body?.kind, text: body?.text, inject: body?.inject })
      } else {
        const key = instance ?? '-'
        const stepEvents = (record.steps[stepId] ??= {})
