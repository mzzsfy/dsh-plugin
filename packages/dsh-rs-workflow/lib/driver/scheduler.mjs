// 就绪批次计算:依赖/for_each 展开/并发上限/审批独占/flow 递归/死锁防护
...
// 有效依赖:显式 after 优先,否则文档序最近的前一个非升级步(escalate-only 不参与链)
export function effectiveDeps(template, escalateOnly) {
  const deps = new Map()
  template.steps.forEach((step, i) => {
    if (escalateOnly.has(step.id)) {
      deps.set(step.id, [])
      return
    }
    if (Array.isArray(step.after)) {
      deps.set(step.id, step.after)
      return
    }
    let prev = null
    for (let j = i - 1; j >= 0; j--) {
      if (!escalateOnly.has(template.steps[j].id)) {
        prev = template.steps[j].id
        break
      }
    }
    deps.set(step.id, prev ? [prev] : [])
  })
  return deps
}
...
// 就绪批次。kind:normal(并行 calls)/approve(独占)/flow(嵌套)/terminal/blocked/idle
export function nextBatch(state, template, { concurrency = DEFAULT_CONCURRENCY } = {}) {
  const escalateOnly = escalateOnlyIds(template)
  const deps = effectiveDeps(template, escalateOnly)

  // 实例失败传播与步骤态归一
  for (const step of template.steps) if (step.for_each) normalizeForEach(state, step)
  ...
  for (const step of template.steps) {
    ...
    const bad = (deps.get(step.id) ?? []).some((d) => state.steps[d] && stepBlockedInput(state.steps[d]))
    if (bad) {
      s.status = 'skipped'
      s.skipReason = '依赖失败传播'
      s.instances = []
  ...
  // 审批独占批次:target done 且依赖 done 才就绪
  for (const step of template.steps) {
    if (stepTypeOf(step) !== 'approve') continue
    const s = state.steps[step.id]
    if (!s || s.status !== 'pending') continue
    const depsOk = (deps.get(step.id) ?? []).every((d) => stepDone(state.steps[d] ?? {}))
  ...
  const anyRunning = template.steps.some((step) => {
    const s = state.steps[step.id]
    if (!s) return false
    return s.status === 'running' || (s.instances ?? []).some((i) => i.status === 'running')
  })
  const anyPending = template.steps.some((step) => {
    const s = state.steps[step.id]
