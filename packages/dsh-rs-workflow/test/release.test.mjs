// release BDD(node --test):全场景临时目录,经参数传 dshHome
...
const ENTRY = {
  ...
  label: '小说',
  description: '小说创作流程\n从大纲到成稿的编排',
  json5: "{ id:'novel', label:'小说', steps:[{ id:'a', slot:'planner', prompt:'{request}', outputs:{ o:'o' } }] }",
}
...
test('Given 预设根 rs-novel(v0.3.0 本包 flow)与 rs-default(v4.0.0) When sweepLegacyReleases Then 前者删后者留且清单含两者', (t) => {
  ...
  writeMarker(home, 'default', FLOW_MARKER(CURRENT_VERSION))
...
test('Given rs-workflow(collab 本包 marker)与 rs-mydir(外来 marker) When sweep Then 前者删后者原样保留', (t) => {
  ...
  writeMarker(home, 'mydir', { package: FOREIGN_PACKAGE, kind: KIND_FLOW, version: CURRENT_VERSION })
  const result = sweepLegacyReleases(home)
...
test('Given 合法模板 entry When releaseFlowTemplate Then created 且四文件产物符合契约', (t) => {
  ...
  assert.equal(readFileSync(join(dir, FLOW_FILE), 'utf8'), ENTRY.json5)
  const agent = readFileSync(join(dir, AGENT_YAML), 'utf8')
  assert.ok(agent.includes("new URL('flow.json5', baseUrl)"))
  assert.ok(!agent.includes('flows/default.json5'))
  assert.ok(agent.includes('prefix: |-'))
  assert.ok(agent.includes('suffix: '))
  assert.ok(!agent.includes('text: |-'))
  assert.ok(agent.includes('无需重发'))
...
test('Given 非法 id 或坏模板 When release Then throw 且校验错误逐条 target:message', (t) => {
  ...
  const brokenJson5 = "{ id:'novel', label:'小说', steps:[{ id:'a', prompt:'{request}' }] }"
  assert.throws(() => releaseFlowTemplate({ ...ENTRY, json5: brokenJson5 }, home), (error) => {
    return error.message.includes('模板校验失败') && error.message.includes('step:a:')
  })
  assert.throws(() => releaseFlowTemplate({ ...ENTRY, json5: '{ id:' }, home), /模板 json5 解析失败/)
})

test('Given 目标目录被外来 marker 占用 When release Then throw 且原目录未动', (t) => {
  const home = tempHome(t)
  putFile(home, 'novel', 'user-file.txt', 'foreign data')
  writeMarker(home, 'novel', { package: FOREIGN_PACKAGE, kind: KIND_FLOW, version: CURRENT_VERSION })
...
test('Given 已创建 When unrelease Then removed 且目录消失;再 unrelease Then missing;外来目录 Then foreign 不删', (t) => {
  ...
  writeMarker(home, 'other', { package: FOREIGN_PACKAGE, kind: KIND_FLOW, version: CURRENT_VERSION })
