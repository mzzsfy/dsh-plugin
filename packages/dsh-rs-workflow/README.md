...
若水工作流 (rs-workflow):通用**强规则驱动工作流编排工具**。目标:把 AI 自驱的工作流变成强规则驱动的工作流——控制权从模型手里收回到引擎手里:弱 AI 在规则兜底下可用(产出不达标不放行),强 AI 的行为被规则约束(每一步只回答被问的问题)。两种形态:
...
选中任一模式后,会话内一切用户消息被 pre-step 拦截进编排,主会话模型零参与——弱模型被流程兜住出活,强模型不自由发挥、行为符合预期。设计原理、BDD 验收场景与 rscli 映射见仓库根 `docs/DESIGN.md`;模板 DSL 规范的唯一真相源是包内 `lib/spec.mjs`(工具 `spec` 动作与看板「模板规范」分区同源输出)。
...
rs 会话(存在编排记录)的会话页签栏(对话/轨迹/…)出现「若水编排」页:只显示**当前会话**的编排,实时细节由宿主监听 `workflow/*` 生命周期事件(phase/log/agent-start/agent-end)写入运行记录,运行中卡片 2 秒自轮询刷新;会话头部同步显示「若水编排 N 运行中」状态胶囊。普通会话无此页签与胶囊,页签随会话切换即时出现/消失。进程重启后遗留的"运行中"记录在新编排启动时统一落定为中断。
...
模式选择器选「若水·协作编码」→ 直接发需求 → 引擎分诊四模板之一 → planner 规划、executor 执行、reviewer 审批 → 看板看进度。运行期间追加消息被忽略(不进编排、不进主模型),完成后可继续提交。
