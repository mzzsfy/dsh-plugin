// rs-workflow client:设置页分区「若水工作流」——流程模板(gui-editor)/配置(gui-config)。
// 自注册 __ModuleLoader__.load;数据经 /api/rsww/* 读写。
...
`
    ...
    function TemplatesPage() {
      ...
      const reload = useCallback(async () => {
        const t = await request('templates')
      ...
      return h('div', { className: 'rsww-root' },
        ...
        React.createElement('div', { className: 'rsww-head' },
          ...
          React.createElement('span', { className: 'rsww-head__caption' },
            '流程模板与配置在此集中管理;模板可创建为可选模式,供编排引擎按流程驱动子代理执行。')),
