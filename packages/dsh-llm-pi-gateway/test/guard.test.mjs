// guard 哨兵行 BDD:死态( gateway 行被禁且官方行仍被 bundle patch 禁用)
// 时动态代挂官方插件恢复服务;gateway 行复活时先卸代挂再放行,零冲突交接。
// 背景:dsh-market 对 carrier 包的禁用写即时生效的 user patch 行,而其恢复
// 机制(整包移出 bundles)只在下次 boot 生效,窗口内 composed = 官方禁 +
// gateway 禁 = 全模型不可用。guard 行 id 带 "/" 使 market 的行写入拒绝,
// 窗口内始终存活,是死态唯一的自愈执行者。
...
test('死态判定: gateway 行禁用(或不存在)且官方行禁用停稳 → 死态', () => {
  ...
  assert.equal(detectDeadState(loaderOf({ official: entryOf({ id: OFFICIAL_ENTRY_ID, disabled: true }) })), true, 'gateway 行不存在按死态处理')
})
...
test('apply: 死态下代挂官方模块并等待其激活', async () => {
  const { ctx, state } = ctxFixture({ loader: loaderOf({ official: entryOf({ id: OFFICIAL_ENTRY_ID, disabled: true }) }) })
  await installGuard(ctx, { importOfficial: async () => OFFICIAL_STUB })
...
test('apply: 官方包缺失 → 干净禁用,不装任何监听', async () => {
  const { ctx, state, callbacks } = ctxFixture({ loader: loaderOf({ official: entryOf({ id: OFFICIAL_ENTRY_ID, disabled: true }) }), officialModule: null })
...
test('复活让位: gateway 行将 init 且正代挂 → 先卸代挂再放行', async () => {
  const { ctx, callbacks, currentMount, state } = ctxFixture({ loader: loaderOf({ official: entryOf({ id: OFFICIAL_ENTRY_ID, disabled: true }) }) })
  await installGuard(ctx, { importOfficial: async () => OFFICIAL_STUB })
  assert.equal(state.plugged, 1)
  let released = false
...
test('禁用感知: partial-dispose 到官方行(手动禁官方铸成死态)→ 自愈代挂', async () => {
  ...
  await installGuard(ctx, { importOfficial: async () => OFFICIAL_STUB, delay: async () => {} })
...
test('复活让位: 非目标行 / gateway 已运行 / 未代挂 → 直接过', async () => {
  const { ctx, callbacks, state } = ctxFixture({ loader: loaderOf({ official: entryOf({ id: OFFICIAL_ENTRY_ID, disabled: true }) }) })
  await installGuard(ctx, { importOfficial: async () => OFFICIAL_STUB })
...
test('死态判定: 官方行缺席(loader 解析失败)→ 非死态,不代挂', () => {
  assert.equal(detectDeadState(loaderOf({ gateway: entryOf({ id: 'llm-pi-gateway', disabled: true }) })), false)
})
...
test('让位: 在途代挂(mounting)未完成时复活 → 等待落定后卸载再放行', async () => {
  ...
  const installing = installGuard(ctx, { importOfficial: async () => OFFICIAL_STUB })
  for (let tick = 0; callbacks.length < 2 && tick < 100; tick += 1) await new Promise((resolve) => setTimeout(resolve, 1))
