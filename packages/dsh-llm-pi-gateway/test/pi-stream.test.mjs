...
test('classifyPiAiError:分支序表(quota 先于 rate limit;与官方同序为人工源码对表结论,本用例钉扎本包行为)', () => {
  ...
  assert.equal(classifyPiAiError('failed to buffer the request body: length limit exceeded'), 'INVALID_REQUEST', '0.84 新 413 语料')
  ...
  assert.equal(classifyPiAiError('the other side closed the connection'), 'TRANSPORT', '0.84 新传输语料')
  assert.equal(classifyPiAiError('HTTP2 request did not get a response'), 'TRANSPORT', '0.84 新传输语料')
  assert.equal(classifyPiAiError('WebSocket closed unexpectedly'), 'TRANSPORT', '0.84 新传输语料')
