...
// pi-ai 上游把捕获错误展平为 message 字符串,只能按文本分类(官方同款局限)。
...
// 上下文超限文本特征:官方依赖 dsh-llm 判定器,此处以文本近似,报告列明。
const CONTEXT_OVERFLOW_PATTERN = /context (?:window|length).*exceed|exceed.*context (?:window|length)|maximum context length/i
 ...
 */
export function mapStopReason(message, callerAborted = false) {
  const effective = callerAborted && message.stopReason === 'error'
    ? { ...message, stopReason: 'aborted' }
    : message
  const text = effective.errorMessage
  if (effective.stopReason === 'error' && text !== undefined && CONTEXT_OVERFLOW_PATTERN.test(text)) {
    return { kind: 'error', failure: { message: text, code: 'CONTEXT_WINDOW_EXCEEDED' } }
  }
  ...
  for await (const event of events) {
    switch (event.type) {
      ...
      case 'error':
        ...
        yield { type: 'finish', reason: mapStopReason(event.error, callerSignal?.aborted === true) }
        return
      default:
        break
    }
