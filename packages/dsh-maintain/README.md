...
- 运行环境检测与升级后自动重启:host 启动时检测运行环境(pm2 / systemd / supervisord / Kubernetes / 容器,依据进程环境变量与 POSIX 容器特征,win32 跳过容器探测;stdin+stdout 双 TTY 判定为手动直跑;环境变量 DSH_MAINTAIN_RUNTIME_ENV=managed|manual 可强制指定,最高优先)。升级成功且复读校验通过时,托管环境下延迟 3 秒自动重启宿主(复用手动重启互斥,调度窗口内升级被拒),浮条提示宿主将自动重启并自动进入重启等待探测,页面随宿主恢复自动刷新;手动直跑环境不自动退出,浮条提示手动重启;检测为未知形态按托管处理(维持自动重启,由重启等待总超时兜底提示)。升级失败或 stale 不调度。自动重启可在面板开关(`autoRestart` 设置,默认开启):关闭后升级完成仅提示,需在面板手动重启,手动直跑的"请手动重启"指引不受该开关影响。
...
- 面板设置编辑:升级命令、轮询间隔、镜像地址、自动重启开关均可在面板直接编辑保存,与 settings.yaml 等效(轮询间隔保存即重排,镜像地址保存即重查);host 侧拒绝空命令、非 http(s) 地址、带查询串/锚点的镜像地址与非 boolean 的开关值(拼接 dist-tags 路径时 query 会吞掉路径),面板清空输入即恢复对应默认值。
...
maintain:
  ...
  autoRestart: true                              # 升级成功后自动重启宿主,关闭后仅提示需手动重启
```

升级命令、轮询间隔、镜像地址、自动重启开关四项亦可在设置面板"版本与运维"页直接编辑保存(等效 settings.yaml,热生效)。
