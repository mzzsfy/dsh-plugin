...
import { readdirSync, readFileSync, writeFileSync, existsSync, rmSync, symlinkSync, mkdirSync } from 'node:fs'
import { join } from 'node:path'
import { REPO_ROOT, DSHMARKET_PIN, workspaceYaml, runCmd, log } from './lib.mjs'
...
function writeFileNoBom(path, content) {
...
export async function buildProfile({ version, workRoot, hostDir }) {
  ...
  writeFileNoBom(join(profileDir, 'package.json'), JSON.stringify(profileManifest(bundles, all), null, 2) + '\n')
  writeFileNoBom(join(profileDir, 'cordis.yml'), '[]\n')
  writeFileNoBom(join(profileDir, 'cordis.patch.yml'), '[]\n')
  writeFileNoBom(join(profileDir, '.npmrc'), 'registry=https://registry.npmjs.org\n')
  // minimumReleaseAge: 0 覆盖全局宽限配置,隔离环境不拦刚发布版本(测试对象经 symlink 指向仓库副本);
  // allowBuilds 与真实 profile 同款:pnpm 11 对未批准的依赖构建脚本按错误处理
  writeFileNoBom(join(profileDir, 'pnpm-workspace.yaml'), workspaceYaml())
  ...
  for (const name of all) {
    ...
    symlinkSync(join(REPO_ROOT, 'packages', shortName), linkPath, type)
  }
